...
namespace grade_management.Areas.Moderator.Controllers
{
    ...
    public class GradeManagementController : Controller
    {
        ...
        public GradeManagementController(ApplicationDbContext context, ILogger<GradeManagementController> logger)
        {
            _context = context;
            _logger = logger;
        }
        ...
        // Display student's subjects management page - MODERATOR CAN EDIT GRADES
        ...
        // Add subject to student - MODERATOR CAN ADD SUBJECT
        [HttpPost]
        ...
        public async Task<IActionResult> AddSubject(string studentId, int subjectId)
        {
            ...
            TempData["SuccessMessage"] = $"Student has been successfully enrolled in {subject.SubjectName}.";
            return RedirectToAction(nameof(ManageSubjects), new { studentId });
        }

        // Display grade modification form - MODERATOR CAN MODIFY GRADES
        ...
        // Process grade modification - MODERATOR CAN MODIFY GRADES
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> ModifyGrade(GradeModel model)
        {
            ...
            if (existingGrade == null)
            {
                ...
                return RedirectToAction(nameof(ManageSubjects), new { studentId = model.StudentID });
            }
            ...
            try
            {
                ...
                return RedirectToAction(nameof(ManageSubjects), new { studentId = existingGrade.StudentID });
            }
            catch (Exception ex)
            {
                // Log the exception using proper logging
                _logger.LogError(ex, "Error updating grade for student {StudentID}, subject {SubjectID}", 
                    existingGrade.StudentID, existingGrade.SubjectID);

                TempData["Error"] = "An error occurred while updating the grade.";
                return View(existingGrade);
            }
        }

        // Delete grade - MODERATOR CAN DELETE GRADES
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> DeleteGrade(string studentId, int subjectId)
        {
            ...
            try
            {
                ...
                await _context.SaveChangesAsync();
                TempData["SuccessMessage"] = $"Student has been successfully removed from {grade.Subject.SubjectName}.";
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error deleting grade for student {StudentID}, subject {SubjectID}", 
                    studentId, subjectId);
                TempData["Error"] = "An error occurred while removing the student from the subject. Please try again.";
            }

            return RedirectToAction(nameof(ManageSubjects), new { studentId });
        }

        // Display grade details (read-only view)
        ...
        public async Task<IActionResult> ExportToExcel(string searchString)
        {
            ...
            var fileName = $"DanhSachSinhVien_Moderator_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx";
